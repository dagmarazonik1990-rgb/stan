import {
  ensureUser,
  summarizeUser,
  canAnalyze,
  increaseUsage,
  saveDecision
} from "./_db.js";
import { baseSystemPrompt, buildUserPrompt } from "./_prompts.js";

function fallbackAnalysis(mode, title, situation) {
  const safeTitle = title || "Ta decyzja";
  if (mode === "quick") {
    return `SITUATION SUMMARY
${safeTitle} wygląda jak sytuacja, w której potrzebujesz szybko oddzielić emocje od faktów.

MAIN RISK
Największym ryzykiem jest decyzja pod wpływem napięcia albo nadziei bez twardych przesłanek.

MOST RATIONAL OPTION
Najbardziej racjonalna będzie opcja, która daje więcej stabilności, spójności i mniej zależy od cudzych obietnic.

ONE KEY QUESTION
Czy wybierasz to, co jest realne, czy to, co chciałabyś, żeby było realne?

DECISION SIGNAL
🟡 Risky`;
  }

  if (mode === "reality") {
    return `KEY SIGNALS
• W tej historii najważniejsze są niespójności między słowami a działaniem.
• Warto zwrócić uwagę, czy problem się powtarza, czy jest jednorazowy.

WHAT THIS USUALLY MEANS
Takie wzorce często oznaczają, że sytuacja jest mniej stabilna, niż chciałabyś wierzyć.

WHAT TO WATCH FOR
Jeśli dalej będziesz ignorować sygnały ostrzegawcze, koszt emocjonalny może rosnąć.

BLIND SPOT
Możliwe, że bronisz nie faktów, tylko swojej nadziei wobec tej sytuacji.

DECISION SIGNAL
🟠 Serious red flags`;
  }

  return `UNDERSTANDING
Z tego, co opisujesz, ${safeTitle.toLowerCase()} wiąże się z napięciem między tym, czego chcesz, a tym, co naprawdę widać w faktach.

REALITY CHECK
Najważniejsze są tu niespójności, koszt odwlekania i ryzyko, że emocje przykrywają ocenę sytuacji.

POSSIBLE SCENARIOS
1. Jeśli nic nie zmienisz, problem może się przeciągać.
2. Jeśli postawisz granice, początek może być trudny, ale obraz sytuacji stanie się jaśniejszy.
3. Jeśli pójdziesz za nadzieją bez dowodów, możesz zapłacić za to czasem i spokojem.

BLIND SPOT
Możliwe, że nie oceniasz tylko tej decyzji — oceniasz też swoją potrzebę bezpieczeństwa, akceptacji albo uniknięcia straty.

FUTURE SELF
Za 6–12 miesięcy Twoje przyszłe ja może bardziej docenić jasność niż chwilowy komfort.

DECISION SIGNAL
🟠 Serious red flags

CLOSING INSIGHT
Najważniejsze pytanie nie brzmi, czego dziś najbardziej chcesz — tylko co jest spójne z rzeczywistością.`;
}

async function generateAnalysis({ mode, title, situation }) {
  const apiKey = process.env.OPENAI_API_KEY;
  const model = process.env.OPENAI_MODEL || "gpt-5-mini";

  if (!apiKey) {
    return fallbackAnalysis(mode, title, situation);
  }

  const response = await fetch("https://api.openai.com/v1/responses", {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${apiKey}`
    },
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: baseSystemPrompt() },
        { role: "user", content: buildUserPrompt({ mode, title, situation }) }
      ]
    })
  });

  if (!response.ok) {
    return fallbackAnalysis(mode, title, situation);
  }

  const data = await response.json();
  return data?.output_text || fallbackAnalysis(mode, title, situation);
}

export default async function handler(req, res) {
  if (req.method !== "POST") {
    return res.status(405).json({ error: "Method not allowed." });
  }

  const { userId, title, situation, mode } = req.body || {};

  if (!userId || !situation) {
    return res.status(400).json({ error: "userId i situation są wymagane." });
  }

  const user = ensureUser(userId);

  if (!canAnalyze(user)) {
    return res.status(402).json({ error: "FREE_LIMIT_REACHED" });
  }

  const analysis = await generateAnalysis({
    mode: mode || "full",
    title: title || "",
    situation
  });

  increaseUsage(user);
  const decision = saveDecision({
    userId,
    title,
    situation,
    mode: mode || "full",
    analysis
  });

  return res.status(200).json({
    user: summarizeUser(user),
    decision
  });
}