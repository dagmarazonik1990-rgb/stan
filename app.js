...
const el = {
  ...
  followupLaterBtn: document.getElementById("followupLaterBtn"),
  ...
  accountPersonaValue: document.getElementById("accountPersonaValue"),
  accountModeValue: document.getElementById("accountModeValue"),
  accountPlanValue: document.getElementById("accountPlanValue"),
  accountUserValue: document.getElementById("accountUserValue"),
  accountPlanBadge: document.getElementById("accountPlanBadge"),
  accountOrbBtn: document.getElementById("accountOrbBtn"),
  accountSemiBtn: document.getElementById("accountSemiBtn"),
  accountUnlockBtn: document.getElementById("accountUnlockBtn"),
  accountNewDemoBtn: document.getElementById("accountNewDemoBtn")
};
...
function scrollToFollowup() {
  el.followupSection?.scrollIntoView({ behavior: "smooth", block: "start" });
}
...
function openAccountPanel() {
  ...
  setStatus("Otworzono konto STAN.");
}
...
function refreshAccountPanel() {
  if (el.accountPersonaValue) {
    el.accountPersonaValue.textContent = currentPersona === "semi" ? "SEMI" : "ORB";
  }

  if (el.accountModeValue) {
    el.accountModeValue.textContent = (selectedMode || DEFAULT_MODE).toUpperCase();
  }

  if (el.accountPlanValue) {
    el.accountPlanValue.textContent = isProUnlocked ? "PRO DEMO" : "FREE";
  }

  if (el.accountPlanBadge) {
    el.accountPlanBadge.textContent = isProUnlocked ? "PRO" : "FREE";
  }

  if (el.accountUserValue) {
    el.accountUserValue.textContent = getUserId();
  }
}
...
function setPersona(nextPersona) {
  ...
  if (currentPersona === "semi") {
    setStatus("SEMI aktywne.");
  } else {
    setStatus("ORB aktywne.");
  }
}
...
el.newDecisionBtn?.addEventListener("click", () => {
  hideAccountPanel();
  scrollToTopSmooth();
  el.situation?.focus();
});
...
el.historyNavBtn?.addEventListener("click", () => {
  hideAccountPanel();
  scrollToHistory();
});

el.accountNavBtn?.addEventListener("click", () => {
  openAccountPanel();
});

el.followupLaterBtn?.addEventListener("click", () => {
  scrollToFollowup();
});
...
el.semiNavBtn?.addEventListener("click", (event) => {
  event.preventDefault();
  event.stopPropagation();
  setPersona("semi");
});
el.togglePersonaBtn?.addEventListener("click", () => {
  togglePersona();
});

el.accountOrbBtn?.addEventListener("click", () => {
  setPersona("orb");
});

el.accountSemiBtn?.addEventListener("click", () => {
  setPersona("semi");
});

el.accountUnlockBtn?.addEventListener("click", async () => {
  await unlockPro();
  refreshAccountPanel();
});

el.accountNewDemoBtn?.addEventListener("click", async () => {
