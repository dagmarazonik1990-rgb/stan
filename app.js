...
async function shareDecisionCard() {
  ...
  try {
    ...
    card.style.background = "radial-gradient(circle at top left, rgba(139,92,246,0.35), transparent 28%), radial-gradient(circle at 85% 28%, rgba(96,165,250,0.18), transparent 20%), linear-gradient(180deg, #160a28 0%, #12081f 40%, #10081a 100%)";
    ...
    card.innerHTML = `
      <div style="display:flex;align-items:center;gap:18px;margin-bottom:28px;">
        <div style="width:62px;height:62px;border-radius:999px;background:radial-gradient(circle at 35% 30%, rgba(255,255,255,0.82), rgba(255,255,255,0) 22%), radial-gradient(circle at 50% 50%, #a78bfa 0%, #7c3aed 45%, #2a1454 78%, #12081f 100%);box-shadow:0 0 30px rgba(139, 92, 246, 0.45), inset 0 -16px 28px rgba(0,0,0,0.28);"></div>
        <div>
          <div style="font-size:62px;font-weight:900;line-height:0.95;letter-spacing:-0.05em;">STAN</div>
      ...
      <div style="font-size:22px;color:rgba(255,255,255,0.55);">STAN (Original) © 2026</div>
    `;
    ...
    canvas.toBlob(async (blob) => {
      ...
      const file = new File([blob], "stan-karta-decyzji.png", { type: "image/png" });

      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          title: "STAN — karta decyzji",
      ...
      a.href = url;
      a.download = "stan-karta-decyzji.png";
